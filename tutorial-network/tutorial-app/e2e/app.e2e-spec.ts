...
describe('Starting tests for tutorial-app', function() {
  ...
  it('website title should be tutorial-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tutorial-app');
    })
  });
  ...
  it('navbar-brand should be tutorial-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tutorial-app');
    });
  });
